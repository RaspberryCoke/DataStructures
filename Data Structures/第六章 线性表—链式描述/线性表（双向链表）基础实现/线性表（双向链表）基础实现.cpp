...
/*
debug:
class 的public继承时，没有在基类后写<T>

误用initializer_list的[]，实际上并没有这样的重载。应该使用const int * p=list.begin();

析构函数写错了，一是忘记写i++，二是忘记边界条件，lastNode需要单独delete。

insert和erase函数写错了，没有考虑到theIndex是firstNode和lastNode的情况，需要单独讨论。
同时还忘了加i++;
